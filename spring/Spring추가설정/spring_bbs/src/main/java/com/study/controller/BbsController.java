...
public class BbsController {
  ...
  public String delete(int bbsno, String passwd, String oldfile) {
    ...
    if (pflag == 1) {
      ...
      if (oldfile != null && !oldfile.equals(""))
        Utility.deleteFile(upDri, oldfile); // 서버의 스토리지 안에서 지우겠다
    }

    if (pflag != 1)
      return "passwdError"; // 비밀번호 오류일때 비밀오류 페이지를 보여준다.
    else if (flag != 1)
      return "error";
    else
      return "redirect:list";
  ...
  public String update(BbsDTO dto, String oldfile) {
    ...
    if (dto.getFilenameMF().getSize() > 0) {
      if (oldfile != null && !oldfile.equals(""))
        Utility.deleteFile(upDir, oldfile); // oldfile이 null이 아니면 삭제하겠다
  ...
  public String read(@PathVariable int bbsno, 
