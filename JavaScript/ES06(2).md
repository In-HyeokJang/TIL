...
* `proxy.test`를 이용해 값을 읽으면 `target`에서 값을 읽어온다.
...
* `target`: 동작을 전달할 객체
* `property`: 프로퍼티 이름
* `receiver`: 타겟 프로퍼티가 getter라면 `receiver`는 `getter`가 호출 될때 `this`이다. 대게 `proxy`객체 자신이 `this`가 된다. `proxy`객체를 상속받은 객체가 있다면 해당 객체가`this`가 된다.
...
let proxy = new Proxy(numvers, handler);
